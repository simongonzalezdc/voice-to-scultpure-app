...
export function applyAmplitudeEnvelope(
	...
	for (let i = profile.length - 1; i >= 0; i--) {
		...
		for (
			...
			j <= Math.min(profile.length - 1, i + smoothWindow);
			j++
		) {
			sum += envelope[j] ?? 0.5;
			count++;
...
export function applyFormModes(profile: LathePoint[], config: FormModeConfig): LathePoint[] {
	...
	result = applyAmplitudeEnvelope(
		result,
		config.envelopeSmoothEnabled,
		config.envelopeSmoothAmount
	);
	result = applyProfileFollowingFins(
		result,
		config.profileFinsEnabled,
		config.profileFinsBaseRadius
	);
