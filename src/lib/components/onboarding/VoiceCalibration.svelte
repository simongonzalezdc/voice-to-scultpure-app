<script lang="ts">
	...
	$effect(() => {
		if (
			...
			if (energy > 0.01) {
				// Only sample when there's audio
...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-lg">
	<div
		class="relative w-full max-w-lg mx-4 overflow-hidden rounded-3xl bg-gradient-to-br {currentConfig.bgGradient} border border-white/10 shadow-2xl"
	>
		...
		<div class="relative z-10 p-8">
			...
			<div class="text-center mb-8">
				<div
					...
					style="transform: scale({1 + energyDisplay * 0.3});"
				>
					{currentConfig.icon}
				</div>
			...
			{#if calibrationStore.step !== 'idle' && calibrationStore.step !== 'complete'}
				<div
					class="relative h-48 mb-8 rounded-2xl bg-black/30 border border-white/5 overflow-hidden"
				>
					...
					<div class="absolute left-4 bottom-0 w-8 bg-black/40 rounded-t-lg overflow-hidden h-full">
						...
						></div>
						<span
							class="absolute bottom-2 left-1/2 -translate-x-1/2 text-xs text-white/50 font-mono"
						>
				...
				<div class="mb-6">
					<div class="flex justify-between text-sm text-white/50 mb-2">
						<span
							>Step {['low-note', 'high-note', 'loud', 'soft'].indexOf(calibrationStore.step) + 1} of
							4</span
						>
					...
					<div class="grid grid-cols-2 gap-4 text-sm">
						<div>
							...
							<p class="text-white font-mono">
								{calibrationStore.result.pitchLow.toFixed(0)} - {calibrationStore.result.pitchHigh.toFixed(
									0
								)} Hz
							</p>
						</div>
						<div>
							...
							<p class="text-white font-mono">
								{(calibrationStore.result.energyFloor * 100).toFixed(0)}% - {(
									calibrationStore.result.energyCeiling * 100
								).toFixed(0)}%
							</p>
						</div>
			...
			{#if calibrationStore.step === 'idle'}
				<div class="mb-8 p-4 rounded-xl bg-black/20 border border-white/5 text-center">
					<p class="text-white/60 text-sm mb-4">
						We'll listen to your voice for about 10 seconds to create a personalized sculpture
						experience.
					</p>
			...
			<div class="flex gap-3">
				...
				{:else if calibrationStore.step === 'processing'}
					<div
						class="flex-1 py-4 px-6 rounded-xl font-semibold text-white/60 bg-white/5 text-center"
					>
			...
			{#if calibrationStore.error}
				<div
					class="mt-4 p-3 rounded-lg bg-red-500/20 border border-red-500/30 text-red-300 text-sm"
				>
