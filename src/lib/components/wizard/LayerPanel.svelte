...
<div class="flex flex-col h-full bg-surface-panel border-l border-white/10 w-64">
	...
	<div class="flex-1 overflow-y-auto p-2 space-y-1">
		{#if layers.length === 0}
			<div class="text-center p-4 text-white/30 text-xs">No active layers</div>
		{:else}
				...
				>
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-2 overflow-hidden">
							...
							<span
								class="text-xs font-mono truncate select-none {activeId === layer.id
									? 'text-brand-primary'
									: 'text-white/80'}"
							>
								{layer.name}
					...
					<div class="flex items-center gap-2 px-1">
						...
						/>
						<span class="text-[10px] text-white/50 w-6 text-right"
