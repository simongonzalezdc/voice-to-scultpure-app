<script lang="ts">
	...
	const particles: Particle[] = Array(COUNT).fill(0).map(() => ({
	...
	useTask((delta) => {
		...
		const isRecording = recordingStore.state === 'recording'; // Only emit when actually applying force? Or just hovering? 
		...
		if (shouldEmit) {
			...
			for (let i = 0; i < COUNT; i++) {
				...
				if (!particles[i].active) {
					...
					particles[i].position.copy(interactionPoint!).add(
						new Vector3(
							(Math.random() - 0.5) * 0.1,
							(Math.random() - 0.5) * 0.1,
							(Math.random() - 0.5) * 0.1
					...
					if (isSubtractive) {
						...
						particles[i].velocity.copy(normal).add(randomDir.multiplyScalar(0.5)).normalize().multiplyScalar(2 + Math.random());
					} else {
						...
						particles[i].velocity.copy(particles[i].position).sub(interactionPoint!).normalize().negate().multiplyScalar(2 + Math.random());
					}
	...
	let particleColor = $derived(
		uiStore.sculptMode === 'subtractive' ? '#ff8800' : '#00ffff'
	);
</script>
...
{#if uiStore.workspace === 'force'}
	<T.InstancedMesh 
		...
		frustumCulled={false}
	>
		<T.IcosahedronGeometry args={[1, 0]} /> <!-- Low poly particles -->
