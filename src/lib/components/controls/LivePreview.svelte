...
<div
	class="live-preview rounded-xl overflow-hidden border border-white/10 bg-black/20 backdrop-blur-sm"
	class:compact
>
	<!-- Header -->
	...
	<div class="flex" class:flex-col={compact}>
		...
		<div class="flex-1 relative" class:h-16={compact} class:h-24={!compact}>
			<canvas bind:this={waveformCanvas} class="absolute inset-0 w-full h-full"></canvas>
		...
		{#if !compact}
			<div class="w-32 border-l border-white/5 relative">
				<canvas bind:this={profileCanvas} class="absolute inset-0 w-full h-full"></canvas>
			</div>
		{/if}
	</div>
